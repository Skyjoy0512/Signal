...
**DB:** Supabase Postgres, スキーマ: supabase/migrations/0001_initial_schema.sql
**LLM:** DeepSeek V4 Pro（推論）/ Flash（ワーカー）
...
src/lib/
  ...
  scoring/        — 機械式スコア + LLM補正 + シグナル分類 + ATR利確損切り
...
- `scoring/signal-detector.ts` — Strong Entry / Entry候補 / 監視 / 見送り の4分類
- `llm/orchestrator.ts` — 推論→検証→修復→Critic のパイプライン
- `mock/provider.ts` — Supabase/APIキー不要のデモモード

## API
- `GET /api/signals` — 現在のシグナル一覧
- `POST /api/jobs/daily-scan` — スキャン実行
- `GET /api/external-pack?symbol=` — 匿名化Markdownダウンロード
- `POST /api/line/webhook` — LINEボット受信

## ページ（全日本語化済み）
`/`（ヒーロー+Unsplash画像+モジュールカード）, `/dashboard`, `/candidates`, `/positions`, `/review`, `/settings`

## 状態
- tsc: エラー0
- テスト: 74件 / 6ファイル 全通過
- 開発サーバー: `npm run dev` → :3001
