...
export interface CriticOutput { critic_pass: boolean; major_concerns: string[]; required_downgrade: boolean; downgrade_to: "entry_candidate" | "watch" | "avoid" | null; reason: string; }
...
export interface ChatCompletionResult { content: string; inputTokens: number; outputTokens: number; estimatedCost: number; latencyMs: number; model: string; }
...
export interface LlmRunResult { status: LlmRunStatus; outputJson: unknown; outputText: string | null; inputTokens: number; outputTokens: number; estimatedCost: number; latencyMs: number; errorMessage: string | null; }
